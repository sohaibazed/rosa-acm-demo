    ...
    operator_iam_roles = [
      {
        ...
        role_arn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.managed_cluster_name}-openshift-cloud-credential-operator-cloud-c",
      },
      {
        name      = "installer-cloud-credentials",
        namespace = "openshift-image-registry",
        role_arn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.managed_cluster_name}-openshift-image-registry-installer-cloud-cr",
      },
      {
        name      = "cloud-credentials",
        namespace = "openshift-ingress-operator",
        role_arn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.managed_cluster_name}-openshift-ingress-operator-cloud-credential",
      },
      {
        name      = "ebs-cloud-credentials",
        namespace = "openshift-cluster-csi-drivers",
        role_arn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.managed_cluster_name}-openshift-cluster-csi-drivers-ebs-cloud-cre",
      },
      {
        name      = "cloud-credentials",
        namespace = "openshift-cloud-network-config-controller",
        role_arn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.managed_cluster_name}-openshift-cloud-network-config-controller-c",
      },
      {
        name      = "aws-cloud-credentials",
        namespace = "openshift-machine-api",
        role_arn  = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/${var.managed_cluster_name}-openshift-machine-api-aws-cloud-credentials",
      },
    ]
    instance_iam_roles = {
...
resource "ocm_cluster" "acm_managed_cluster" {
  product        = "rosa"
  cloud_provider = "aws"
  ...
  wait = false
  sts  = var.enable_sts ? local.managed_sts_roles : null
...
module "managed_sts_roles" {
  count                = var.enable_sts ? 1 : 0
  source               = "rh-mobb/rosa-sts-roles/aws"
  create_account_roles = false
  clusters = [{
    id                   = ocm_cluster.acm_managed_cluster.id
    operator_role_prefix = var.managed_cluster_name
  }
  ]
}

resource "ocm_identity_provider" "managed_cluster_rosa_iam_htpasswd" {
   cluster = ocm_cluster.acm_managed_cluster.id
   name = "htpasswd"
   htpasswd = {
...
resource "ocm_group_membership" "managed_cluster_htpasswd_admin" {
  cluster = ocm_cluster.acm_managed_cluster.id
